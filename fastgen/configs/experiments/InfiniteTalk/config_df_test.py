...
from fastgen.datasets.infinitetalk_dataloader import create_infinitetalk_dataloader
...
class StdoutLoggerCallback(Callback):
    ...
    def on_training_step_end(self, model, data, outputs, losses, iteration, **kwargs):
        if losses:
...
def create_config():
    ...
    config.trainer.validation_iter = 0  # no validation
    ...
    config.dataloader_train = L(create_infinitetalk_dataloader)(
