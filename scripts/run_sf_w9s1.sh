#!/bin/bash
# InfiniteTalk Self-Forcing Training — Fixed Attention (window=9, sink=1)
#
...
# (one of the five configs from config_df_quarter_stochastic.py).
#
# Teacher and fake_score remain bidirectional.
#
# Usage:
#   bash scripts/run_sf_w9s1.sh
#   # Or with soft-anchor modes:
#   STUDENT_ANCHOR_EVAL_ONLY=1 bash scripts/run_sf_w9s1.sh
#   STUDENT_ANCHOR_EVAL_ONLY=1 FAKE_SCORE_ANCHOR_EVAL_ONLY=1 bash scripts/run_sf_w9s1.sh
...
echo "============================================"
echo "InfiniteTalk SF Training — w9s1 (window=9, sink=1)"
echo "============================================"
echo "GPUs:             $NUM_GPUS"
...
torchrun --nproc_per_node=$NUM_GPUS \
    train.py \
    --config fastgen/configs/experiments/InfiniteTalk/config_sf_w9s1.py
