#!/bin/bash
# InfiniteTalk Self-Forcing Training — Train-Time Anchor-Free + w9s1
#
# During training:    no model anchors
#   - Student:     soft (generates frame 0 from noise, learns via VSD)
#   - Fake score:  soft (tracks student's anchor-free distribution)
#   - Teacher:     disabled (target distribution has anchor-free frame 0 too,
#                  avoiding train/eval distribution mismatch for VSD gradients)
#
# During validation / inference:
#   - Student:     anchor active (reference image replaces frame 0 → I2V output)
#   - Teacher / fake_score are not used during eval.
#
# This is softboth + teacher_anchor_disabled.
#
# Usage:
...
echo "============================================================"
echo "InfiniteTalk SF Training — w9s1 + ANCHOR-FREE"
echo "============================================================"
echo "GPUs:             $NUM_GPUS"
echo "Student attn:     local_attn_size=10, sink_size=1"
echo "Anchor (train):   OFF for student, teacher, fake_score"
echo "Anchor (eval):    ON for student (teacher/fake_score unused at eval)"
...
torchrun --nproc_per_node=$NUM_GPUS \
    train.py \
    --config fastgen/configs/experiments/InfiniteTalk/config_sf_w9s1_noanchor.py
