#!/bin/bash
# InfiniteTalk Self-Forcing Training — Soft Anchor Mode
#
# Student learns frame 0 via VSD gradient (no anchor during training rollout).
# Teacher and fake_score always anchor frame 0 → clean VSD targets.
# Student anchors frame 0 at inference/validation only.
#
# This matches the conventional I2V paradigm:
#   Train: soft conditioning only (model learns frame 0)
#   Infer: anchor for quality (inference-time trick)
#
# Usage:
#   bash scripts/run_sf_softanchor_quarter.sh

set -e

# ── Enable soft anchor mode ──
...
echo "================================================"
echo "InfiniteTalk SF Training — Soft Anchor Mode"
echo "================================================"
echo "GPUs:             $NUM_GPUS"
echo "Student anchor:   eval-only (learns frame 0 via VSD)"
echo "Teacher anchor:   always"
echo "Fake_score anchor: always"
