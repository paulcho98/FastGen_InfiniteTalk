#!/bin/bash
# InfiniteTalk Diffusion Forcing — 8-GPU DDP production training (v2)
#
# Fixes from v1:
#   - expandable_segments to reduce CUDA memory fragmentation (was OOM at iter 8)
#   - Fresh wandb run (WANDB_RUN_ID unset, new output dir)
#
# LoRA rank 128, alpha 64, LR 1e-5, grad_accum 4 → effective batch size 32
# 147K samples with lazy caching (T5 pre-computed, VAE/CLIP/audio on GPU)
#
# Usage:
#   bash scripts/run_df_training_8gpu_v2.sh
...
# Ensure fresh run — unset any stale run ID
unset WANDB_RUN_ID

# ── CUDA memory management ──
# expandable_segments reduces fragmentation (was OOM with 6.76GB reserved-but-unusable)
