#!/bin/bash
# InfiniteTalk Self-Forcing — Validation-Only Test (w9s1 + Lookahead)
#
...
# end-to-end.
#
# Lookahead (F1) enabled by default. F2/F3 via env vars (same as training script).
#
...
#
# Expected wandb output (under group "infinitetalk_sf_w9s1_lookahead_valtest"):
...
#   LOOKAHEAD_DISTANCE=6 bash scripts/run_sf_w9s1_lookahead_valtest.sh
...
echo "============================================"
echo "InfiniteTalk SF Validation-Only Test — w9s1 + Lookahead"
echo "============================================"
echo "GPUs:             $NUM_GPUS"
...
echo "Wandb group:      infinitetalk_sf_w9s1_lookahead_valtest"
