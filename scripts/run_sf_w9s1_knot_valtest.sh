#!/bin/bash
# InfiniteTalk Self-Forcing — Validation-Only Smoke Test (w9s1 + KF + RA + Last-ref)
#
# Runs validation at iter 0 and iter 1, then exits. No training updates.
# Uses a 2-sample val list for speed. Wandb online so outputs are visible.
#
...
#
# Expected wandb output (under group "infinitetalk_sf_w9s1_knot_valtest"):
#   - val0/generated          (2 student AR-generated videos under KF path)
#   - val0/total_loss         (scalar)
#   - val1/generated          (2 more from the final-iter validation)
#
# Usage:
#   bash scripts/run_sf_w9s1_knot_valtest.sh
#   KNOT_SIZE=2 bash scripts/run_sf_w9s1_knot_valtest.sh
...
export INFINITETALK_TRAIN_LIST="${INFINITETALK_TRAIN_LIST:-data/precomputed_talkvid/val_quarter_2.txt}"
...
# NOTE: valtest disables lazy caching inside the config (sets raw_data_root=None)
...
echo "============================================"
echo "InfiniteTalk SF Valtest — w9s1 + KF + RA + Last-ref"
echo "============================================"
echo "GPUs:             $NUM_GPUS"
echo "Student attn:     local_attn_size=10, sink_size=1"
echo "Dynamic RoPE:     ON"
echo "Temporal Knot:    ON (k=${_K})"
...
echo "Wandb group:      infinitetalk_sf_w9s1_knot_valtest"
...
torchrun --nproc_per_node=$NUM_GPUS \
    train.py \
    --config fastgen/configs/experiments/InfiniteTalk/config_sf_w9s1_knot_valtest.py
